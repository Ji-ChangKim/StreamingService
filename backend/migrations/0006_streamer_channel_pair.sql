-- Migration 0006: Intuitive Pair Tables (streamerChannel & streamerChannel_info)

DROP TABLE IF EXISTS streamer_channels;
DROP TABLE IF EXISTS streamers;
DROP TABLE IF EXISTS channel_management;
DROP TABLE IF EXISTS streamer_info;
DROP TABLE IF EXISTS streamerChannel_info;
DROP TABLE IF EXISTS streamerChannel;

-- ① 상위 메인 테이블: streamerChannel (방송국 / 라이브 주소 메인)
...
-- ② 하위 참조 세부 정보 테이블: streamerChannel_info (channel_id 1:1 UNIQUE 참조)
CREATE TABLE streamerChannel_info (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  channel_id INTEGER NOT NULL UNIQUE, -- streamerChannel.id와 1:1 UNIQUE 참조!
  ...
  FOREIGN KEY (channel_id) REFERENCES streamerChannel(id) ON DELETE CASCADE
);
...
INSERT INTO streamerChannel_info (id, channel_id, slug, display_name, profile_image_url, description, agency_name, debut_date, debut_time, timezone, start_at_utc)
VALUES 
