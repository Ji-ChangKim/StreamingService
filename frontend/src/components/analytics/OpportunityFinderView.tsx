        ...
        <form onSubmit={handleRunSearch} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            ...
            <div className="space-y-1.5">
              <label className="font-semibold text-[#334155]">목표 플랫폼</label>
              <select
                value={platform}
                onChange={(e) => setPlatform(e.target.value as PlatformFilter)}
                className="w-full bg-[#F8FAFC] border border-[#CBD5E1] rounded-xl px-3 py-2 text-[#0F172A] focus:outline-none focus:border-[#2563EB]"
              >
            ...
            {/* 희망 콘텐츠 */}
            <div className="space-y-1.5">
              <label className="font-semibold text-[#334155]">주요 방송 콘텐츠</label>
              <select
                value={category}
            ...
            {/* 방송 길이 */}
            <div className="space-y-1.5">
              <label className="font-semibold text-[#334155]">예상 1회 방송 시간</label>
              <select
                value={duration}
            ...
            {/* 채널 규모 */}
            <div className="space-y-1.5">
              <label className="font-semibold text-[#334155]">현재 내 채널 상태</label>
              <select
                value={creatorTier}
          ...
          {/* 방송 가능 요일 선택 */}
          <div>
            <label className="text-xs font-semibold text-[#334155] block mb-2">
              방송 가능한 요일 (복수 선택)
            </label>
          ...
          <div className="pt-2 flex justify-end">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[#2563EB] hover:bg-blue-700 text-white font-bold text-xs shadow-sm transition-all cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {results.map((rec) => (
            <div
              key={rec.rank}
              className="relative bg-white border border-[#E2E8F0] hover:border-blue-400 rounded-2xl p-5 shadow-sm flex flex-col justify-between transition-all"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="flex items-center gap-1 text-[11px] font-black px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">
                    추천 {rec.rank}순위
                  </span>
                  <div className="text-right">
                    <span className="text-[10px] text-[#64748B] block">기회 점수</span>
                    <span className="text-base font-black text-emerald-600">{rec.score}점</span>
                  </div>
                </div>

                <h4 className="text-base font-black text-[#0F172A] mb-2">{rec.slot}</h4>

                {/* 지표 수치 요약 */}
                <div className="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-[11px] mb-3 font-mono">
                  <div>
                    <span className="text-[#64748B] block text-[10px]">수요(동시시청)</span>
                    <span className="text-[#0F172A] font-bold">{rec.stats.avgViewers.toLocaleString()}명</span>
                  </div>
                  <div>
                    <span className="text-[#64748B] block text-[10px]">방송당 시청</span>
                    <span className="text-emerald-600 font-bold">{rec.stats.viewersPerLive}명/방</span>
                  </div>
                  <div>
                    <span className="text-[#64748B] block text-[10px]">경쟁 LIVE</span>
                    <span className="text-purple-700 font-bold">{rec.stats.avgLiveCount}개</span>
                  </div>
                  <div>
                    <span className="text-[#64748B] block text-[10px]">상위 10 집중도</span>
                    <span className="text-amber-700 font-bold">{Math.round(rec.stats.top10Concentration * 100)}%</span>
                  </div>
                </div>

                {/* 추천 근거 */}
                <div className="space-y-1.5 mb-3">
                  <span className="text-[11px] font-bold text-[#2563EB] block">추천 근거</span>
                  {rec.reasons.map((r, i) => (
                    <div key={i} className="text-[11px] text-[#334155] flex items-start gap-1.5 leading-snug">
                ...
                {rec.cautions && rec.cautions.length > 0 && (
                  <div className="space-y-1 mb-3 pt-2 border-t border-slate-100">
                    ...
                    {rec.cautions.map((c, i) => (
                      <p key={i} className="text-[10px] text-[#64748B] leading-tight">
                        • {c}
                      </p>
                    ))}
                  </div>
              ...
              {rec.comparisonWithUserTime && (
                <div className="mt-3 p-2.5 rounded-xl bg-blue-50/80 border border-blue-200 text-[10px] text-[#334155]">
                  <span className="font-bold text-[#2563EB]">비교 분석: </span>
