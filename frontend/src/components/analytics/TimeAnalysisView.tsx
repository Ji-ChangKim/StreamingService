  ...
  const getCellColor = (cell: HeatmapCell) => {
    switch (activeMetric) {
      case 'opportunity': {
        ...
        if (s >= 80) return 'bg-emerald-500 text-black';
        if (s >= 70) return 'bg-teal-500/90 text-white';
        if (s >= 60) return 'bg-indigo-600/90 text-white';
        if (s >= 50) return 'bg-indigo-900/60 text-gray-300';
        return 'bg-white/5 text-gray-400';
      }
      case 'viewersPerLive': {
        const v = cell.viewersPerLive;
        if (v >= 450) return 'bg-emerald-500 text-black';
        if (v >= 350) return 'bg-teal-500 text-white';
        if (v >= 250) return 'bg-indigo-600 text-white';
        if (v >= 150) return 'bg-indigo-900 text-gray-300';
        return 'bg-white/5 text-gray-400';
      }
      case 'viewers': {
        const v = cell.viewers;
        if (v >= 40000) return 'bg-violet-600 text-white';
        if (v >= 30000) return 'bg-indigo-600 text-white';
        if (v >= 20000) return 'bg-indigo-800 text-gray-200';
        if (v >= 10000) return 'bg-indigo-950 text-gray-300';
        return 'bg-white/5 text-gray-400';
      }
      case 'liveCount': {
        const l = cell.liveCount;
        if (l >= 120) return 'bg-rose-600/80 text-white';
        if (l >= 90) return 'bg-amber-600/80 text-white';
        if (l >= 60) return 'bg-indigo-700 text-white';
        return 'bg-white/5 text-gray-400';
      }
      case 'top10Share': {
        const t = cell.top10Share;
        if (t <= 0.35) return 'bg-emerald-500 text-black'; // 분산됨 (좋음)
        if (t <= 0.45) return 'bg-teal-600 text-white';
        if (t <= 0.55) return 'bg-amber-600 text-white';
        return 'bg-rose-600 text-white'; // 대형 쏠림 심함
      }
      default:
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#131627]/90 border border-white/10 rounded-2xl p-5 shadow-xl">
        ...
        <div className="overflow-x-auto pb-2">
          <div className="min-w-[700px]">
            {/* 시간 라벨 (0~23) */}
            <div className="grid grid-cols-[40px_repeat(24,_1fr)] gap-1 mb-1.5 text-[10px] text-gray-400 font-mono text-center">
              <div />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 선택 시간대 정밀 상세 패널 */}
        {currentCell && (
          <div className="bg-[#141829] border border-indigo-500/30 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between gap-2 pb-3 border-b border-white/10 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-gray-400">표본 일수: 28일 축적</span>
                  </div>
                </div>

                <div className="text-right">
                  <span className="text-[10px] font-bold text-gray-400 block">기회 점수</span>
              ...
              <div className="space-y-2.5 text-xs">
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-white/5">
                  ...
                  <strong className="text-white font-mono">{currentCell.viewers.toLocaleString()}명</strong>
                </div>
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-white/5">
                  ...
                  <strong className="text-white font-mono">{currentCell.liveCount}채널</strong>
                </div>
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-white/5">
                  ...
                  <strong className="text-emerald-400 font-mono">{currentCell.viewersPerLive}명/방</strong>
                </div>
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-white/5">
                  ...
                  <strong className="text-amber-400 font-mono">{Math.round(currentCell.top10Share * 100)}%</strong>
                </div>
              </div>
            </div>

            {/* 판단 코멘트 */}
            <div className="mt-4 p-3 rounded-xl bg-indigo-950/40 border border-indigo-500/20 text-[11px] text-gray-300 leading-relaxed">
              <span className="font-bold text-indigo-300">💡 편성 판단: </span>
        ...
        <div className="lg:col-span-2 bg-[#131627]/90 border border-white/10 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-black/40 border border-white/10">
                <div className="text-xs font-bold text-indigo-300 mb-2">평일 (월~금) 특징</div>
                <ul className="text-xs text-gray-300 space-y-1.5">
                  <li>• 골든타임: <strong className="text-white">21:00 ~ 24:00</strong> 집중</li>
              ...
              <div className="p-4 rounded-xl bg-black/40 border border-white/10">
                <div className="text-xs font-bold text-emerald-300 mb-2">주말 (토·일) 특징</div>
                <ul className="text-xs text-gray-300 space-y-1.5">
                  <li>• 골든타임: <strong className="text-white">14:00 ~ 03:00</strong> 장기 유지</li>
          ...
          <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-xs text-gray-400">
            ...
            <span className="text-indigo-400 font-semibold">반복 편성 설계에 활용 권장</span>
          </div>
        </div>
      </div>
    </div>
