  ...
  return (
    <div className="py-6 sm:py-8 space-y-6 animate-fadeIn max-w-[1200px] mx-auto px-2 sm:px-4">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3 sm:p-4 rounded-[12px] border border-slate-200 shadow-2xs">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* 템플릿 양식 다운로드 버튼 그룹 */}
              <div className="flex items-center bg-slate-100 p-0.5 rounded-[8px] border border-slate-200">
                <button
                  type="button"
                  onClick={downloadExcelTemplate}
                  className="px-2.5 py-1 text-[11px] font-extrabold text-emerald-800 hover:bg-white rounded-[6px] transition-all flex items-center gap-1 cursor-pointer"
                  title="표준 엑셀 템플릿 다운로드"
                >
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-[11px] font-extrabold text-slate-700 hover:bg-white rounded-[6px] transition-all flex items-center gap-1 cursor-pointer"
                  title="표준 CSV 템플릿 다운로드"
                >
              ...
              {/* 엑셀/CSV 일괄 업로드 모달 열기 버튼 */}
              <button
                type="button"
                onClick={() => setIsUploadModalOpen(true)}
                className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-[8px] transition-all shadow-xs flex items-center gap-1.5 cursor-pointer"
              >
      ...
      <SubmissionUploadModal
        ...
        onSuccess={(count) => {
          setActionSuccessMsg(`총 ${count}건의 데뷔 신청서가 심사 대기열에 성공적으로 등록되었습니다.`);
          setTimeout(() => setActionSuccessMsg(null), 4000);
